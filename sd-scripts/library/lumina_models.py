...
#     return TimestepEmbedder.timestep_embedding(t, dim, max_period)
# # ************
...
# 注入controlnet，加入对 controlnet_residuals 的支持，使其能接收来自 ControlNetLumina 的中间层残差信号并注入主干网络
    ...
    def forward(
        ...
        cap_feats: Tensor,
        cap_mask: Tensor,
        controlnet_residuals: Optional[Tuple[Tensor, ...]] = None,
    ) -> Tensor:
        """
        Forward pass of NextDiT.
        Args:
            ...
            cap_feats: (N, L, D) caption features
            cap_mask: (N, L) caption attention mask
            controlnet_residuals: Optional tuple of (N, T, D) tensors from ControlNet,
                                  one per main transformer layer. If provided,
                                  added to the hidden states after each layer.
        Returns:
            x: (N, C, H, W) denoised latents
        """
        _, _, height, width = x.shape  # B, C, H, W
        ...
        if not self.blocks_to_swap:
            for i, layer in enumerate(self.layers):
                ...
                if controlnet_residuals is not None:
                    # Inject control signal from ControlNet
            ...
            for block_idx, layer in enumerate(self.layers):
                ...
                x = layer(x, mask, freqs_cis, t)
                if controlnet_residuals is not None:
                    x = x + controlnet_residuals[block_idx]
                self.offloader_main.submit_move_blocks(self.layers, block_idx)
...
class ControlNetLumina(nn.Module):
    """
    A ControlNet implementation compatible with Lumina's NextDiT architecture.
    
    It:
      - shares the same patchify/embed logic
      - uses the same JointTransformerBlock layers as the base model
      - outputs control residuals at each transformer layer
      - injects control condition via early latent-space conditioning
    
    Args:
        base_model (NextDiT): The base Lumina model to copy architecture from.
        conditioning_channels (int): Number of channels in control condition (e.g., 3 for RGB, 1 for grayscale).
    """
    def __init__(self, base_model: NextDiT, conditioning_channels: int = 3):
        super().__init__()
        ...
        self.n_kv_heads = base_model.n_heads  # Lumina uses GQA, but we copy from base
        self.multiple_of = 256  # hard-coded, matches base
        self.ffn_dim_multiplier = None
        self.norm_eps = base_model.norm_eps
        self.qk_norm = any(hasattr(block.attention, 'q_norm') and not isinstance(block.attention.q_norm, nn.Identity) for block in base_model.layers)
        self.use_flash_attn = any(getattr(block.attention, 'use_flash_attn', False) for block in base_model.layers)
        self.use_sage_attn = any(getattr(block.attention, 'use_sage_attn', False) for block in base_model.layers)
        self.axes_dims = base_model.axes_dims
        self.axes_lens = base_model.axes_lens

        # Copy embedding components (shared or reinitialized)
        self.t_embedder = base_model.t_embedder
        self.cap_embedder = base_model.cap_embedder

        # Control condition encoder (to same latent space as image)
        self.controlnet_cond_embedding = nn.Sequential(
            ...
            nn.SiLU(),
            nn.Conv2d(64, base_model.in_channels, kernel_size=3, stride=2, padding=1),
        )

        # Copy context/noise refiners (optional: you may freeze or share)
        self.context_refiner = nn.ModuleList([
            JointTransformerBlock(
                layer.layer_id,
                layer.dim,
                layer.attention.n_local_heads,
                ...
                use_sage_attn=self.use_sage_attn,
            ) for layer in base_model.context_refiner
        ])
        self.noise_refiner = nn.ModuleList([
            JointTransformerBlock(
                layer.layer_id,
                ...
                use_flash_attn=self.use_flash_attn,
                use_sage_attn=self.use_sage_attn,
            ) for layer in base_model.noise_refiner
        ])

        # Main transformer layers (copy structure)
        self.layers = nn.ModuleList([
            JointTransformerBlock(
                layer.layer_id,
                layer.dim,
        ...
        # ControlNet-specific zero-initialized output heads
        self.controlnet_blocks = nn.ModuleList([
            zero_module(nn.Linear(self.dim, self.dim)) for _ in range(len(self.layers))
        ])

        # RoPE embedder (shared logic)
        self.rope_embedder = RopeEmbedder(axes_dims=self.axes_dims, axes_lens=self.axes_lens)

        # x embedder (reinitialized)
        self.x_embedder = nn.Linear(
        ...
        self.gradient_checkpointing = False # <-- 添加的行

    # vvvvvv 添加的新函数 vvvvvv
    def enable_gradient_checkpointing(self):
        """
        为模型的 Transformer 模块激活梯度检查点以节省内存。
        """
        self.gradient_checkpointing = True
        # 遍历所有包含 JointTransformerBlock 的模块列表，并启用它们的梯度检查点
        ...
        for block in self.layers:
            block.enable_gradient_checkpointing()
        print("ControlNetLumina: Gradient checkpointing has been enabled.")

    def patchify_and_embed_control(
        self,
        x: Tensor,
        ...
        """
        Same as base model's patchify_and_embed, but injects controlnet_cond early.
        """
        bsz, channels, height, width = x.shape
        pH = pW = self.patch_size
        device = x.device

        # Encode control condition to latent space and add to x
        cond_latents = self.controlnet_cond_embedding(controlnet_cond)  # (B, C, H//4, W//4)
        # Resize to match x if needed (assume VAE latent size matches)
        if cond_latents.shape[2:] != x.shape[2:]:
            cond_latents = F.interpolate(cond_latents, size=(height, width), mode='bilinear')
        x = x + cond_latents  # early injection
        ...
        return joint_hidden_states, attention_mask, freqs_cis, l_effective_cap_len, seq_lengths
    ...
    def forward(
        ...
        """
        Forward pass of ControlNetLumina.
        
        Args:
            x: (B, C, H, W) — noisy latents
            t: (B,) — timesteps
            cap_feats: (B, L, D_cap) — caption features
            cap_mask: (B, L) — caption mask
            controlnet_cond: (B, C_cond, H_cond, W_cond) — control image (e.g., canny, depth)
        
        Returns:
            controlnet_block_samples: tuple of (B, T, D) tensors, one per main layer
            controlnet_single_block_samples: None (Lumina has no single blocks)
        """
        t_emb = self.t_embedder(t)
        cap_feats = self.cap_embedder(cap_feats)

        x, mask, freqs_cis, _, _ = self.patchify_and_embed_control(x, controlnet_cond, cap_feats, cap_mask, t_emb)

        control_block_samples = ()
        for i, layer in enumerate(self.layers):
            x = layer(x, mask, freqs_cis, t_emb)
            control_block_samples += (self.controlnet_blocks[i](x),)

        return control_block_samples, None  # match Flux-style return but single_blocks = None
...
# The original 'configs' dictionary was incorrect and did not match the
# LuminaParams or NextDiT model structure. It has been replaced with a
# functional dictionary mapping model names to their correct LuminaParams configurations.

# Note: The 'SimpleNamespace' import is no longer needed for this corrected config,
# but it's kept here if other parts of a project might rely on it.


from types import SimpleNamespace

configs = {
    "Lumina-2B": LuminaParams.get_2b_config(),
    "Lumina-7B": LuminaParams.get_7b_config(),
    "Lumina-3B": LuminaParams(
        patch_size=2,
        dim=2592,
        n_layers=30,
        n_heads=24,
        ...
        axes_dims=[36, 36, 36],
        axes_lens=[300, 512, 512],
        in_channels=4,          # Assuming default VAE
        cap_feat_dim=2304       # Assuming Gemma-2B size like the 2B model
    ),
    "Lumina-4B": LuminaParams(
        patch_size=2,
        dim=2880,
        n_layers=32,
        n_heads=24,
        ...
        axes_lens=[300, 512, 512],
        in_channels=4,          # Assuming default VAE
        cap_feat_dim=3584       # Assuming a proportional caption feature dimension
    ),
}

# --- Example of how to use the corrected configs ---
#
# To create a 2B model instance:
#
# model_params = configs["Lumina-2B"]
# model = NextDiT(
...
#     axes_dims=model_params.axes_dims,
#     axes_lens=model_params.axes_lens,
#     qk_norm=model_params.qk_norm,
#     cap_feat_dim=model_params.cap_feat_dim,
# )
#
# Or, more conveniently using the factory function:
#
# model = NextDiT_2B_GQA_patch2_Adaln_Refiner(params=configs["Lumina-2B"])
#
# print(f"Successfully created a Lumina-2B model with {model.parameter_count() / 1e9:.2f}B parameters.")
