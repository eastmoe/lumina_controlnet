...
def load_controlnet(
    ...
    """
    Load the Lumina ControlNet from the checkpoint path.

    Args:
        ckpt_path (Optional[str]): Path to the checkpoint. If None, returns uninitialized ControlNet.
        dtype (torch.dtype): The data type for the model.
        device (Union[str, torch.device]): The device to load the model on.
        base_model (lumina_models.NextDiT): The base NextDiT model (required for ControlNet initialization).
        conditioning_channels (int, optional): Number of channels in control condition (e.g., 3 for RGB). Defaults to 3.
        disable_mmap (bool, optional): Whether to disable mmap. Defaults to False.

    Returns:
        controlnet (lumina_models.ControlNetLumina): The loaded ControlNet model.
    """
    logger.info("Building Lumina ControlNet")
    
    # Build on meta device to avoid memory allocation
    with torch.device("meta"):
    ...
    if ckpt_path is not None:
        ...
        logger.info("No checkpoint provided, using randomly initialized ControlNet")
